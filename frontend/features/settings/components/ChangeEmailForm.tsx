...
import { useProfile, useUpdateEmail } from "../hooks/useSettings"
...
export function ChangeEmailForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    mutate(
      { email: email.trim(), current_password: password },
      {
        onSuccess: () => {
          setSuccess(true)
          setEmail("")
    ...
    <div className="bg-white rounded-2xl border border-[#E8E0D7] p-6 sm:p-8 max-w-md">
      <div className="mb-6">
        <p className="text-sm text-[#9C8F87]">
          Current email:{" "}
          <span className="font-medium text-[#2A2522]">{profile?.email ?? "—"}</span>
        </p>
      </div>

      {success && (
        <div className="mb-4 rounded-lg bg-[#DCFCE7] border border-[#16A34A]/20 px-4 py-3">
          <p className="text-sm text-[#166534]">Email updated successfully.</p>
        </div>
      )}
